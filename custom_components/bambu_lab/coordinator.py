...
class BambuDataUpdateCoordinator(DataUpdateCoordinator):
    ...
    async def _async_update_data(self):
        LOGGER.debug(
            f"{self.config_entry.data['device_type']} HA POLL: MQTT connected: {self.client.connected}"
        )
    ...
    def _update_hms(self):
        ...
        if device.hms.error_count == 0:
            ...
            for index in range(device.hms.error_count):
                ...
                event_data["hms_code"] = device.hms.errors[f"{index+1}-Error"][
                    : device.hms.errors[f"{index+1}-Error"].index(":")
                ]
                event_data["description"] = device.hms.errors[f"{index+1}-Error"][
                    device.hms.errors[f"{index+1}-Error"].index(":") + 2 :
                ]
        ...
        if not self._updatedDevice:
            ...
            if new_sw_ver != "unknown":
                ...
                dev_reg.async_update_device(
                    hadevice.id, sw_version=new_sw_ver, hw_version=new_hw_ver
                )
    ...
    def _update_ams_info(self):
        ...
        for index in range(0, len(device.ams.data)):
            if device.ams.data[index] is not None:
                ...
                hadevice = dev_reg.async_get_or_create(
